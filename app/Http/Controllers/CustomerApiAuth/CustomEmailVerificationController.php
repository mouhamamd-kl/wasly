...
class CustomEmailVerificationController extends Controller
{
    ...
    {
        $user = Customer::where('id', $request->id)->firstOrFail();
        $msg = $request->user('customer')->hasVerifiedEmail()
    ...
    {
        $user = Customer::where('verification_token', $request->token)->firstOrFail();
