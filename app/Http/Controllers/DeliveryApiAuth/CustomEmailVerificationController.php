...
class CustomEmailVerificationController extends Controller
{
    ...
    {
        $user = Delivery::where('id', $request->id)->firstOrFail();
        $msg = $request->user(Constants::delivery_guard)->hasVerifiedEmail()
    ...
    {
        $user = Delivery::where('verification_token', $request->token)->first();
        if (!$user) {
    ...
    {
        $user = Delivery::where('verification_token', $request->token)->firstOrFail();
